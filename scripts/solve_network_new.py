...
# NEW NUMBERS!
# base_investment = 27437596162.361057 ZAR (solar and wind) #base_investment_solar_wind = (solar_wind_generators.p_nom_opt * solar_wind_generators.capital_cost).sum()
# emissions = 158816374.82439235 tCO2
# carbon_taxes = 84202853768.14459 ZAR (530.19 ZAR/tCO2) - 4.76 billion USD
# total investment = base_investment + carbon taxes
total_investment = 111640449930.50565 #ZAR
...
def add_carbontax_constraints(n, year=2030, total_investment=1e6):  # Example total investment
    ...
    total_investment = 111640449930.50565 #ZAR - 112 billion ZAR, 6.6 billion USD
...
# 0. Add emission prices to marginal costs - for base with emission prices
# Uncomment in solve_network line 689 when not used!
...
def calculate_and_print_emissions_and_taxes(n, iteration, ep=None):
    ...
    # Calculate total physical emissions by summing the products of generation and emission intensity
    total_emissions = (n.generators_t.p / n.generators.efficiency * n.generators.carrier.map(n.carriers['co2_emissions'])).sum().sum()
    ...
    carbon_taxes_zar = (total_emissions * 530.19)  # ZAR tax on emissions
    carbon_taxes_mzar = (total_emissions_mt * 530.19)  # ZAR tax on emissions
    carbon_taxes_musd = (total_emissions_mt * 30)   # USD tax on emissions

    # Print the results for monitoring purposes
    print("\033[91mTotal CO2 emissions:\033[0m", total_emissions_mt, "Mt")
    print("\033[91mCarbon taxes:\033[0m", carbon_taxes_mzar, "M ZAR")
    print("\033[91mCarbon taxes:\033[0m", carbon_taxes_musd, "M USD")


    return total_emissions_mt, carbon_taxes_zar, ep
...
def apply_reinvestment_to_renewables(n, reinvestment_amount):
    ...
    print(f"Total investment of {total_investment} ZAR applied to renewable generators.")
...
#     total_emissions, carbon_taxes_zar, _ = calculate_and_print_emissions_and_taxes(n, iteration=0)

#     # Calculate total investment
#     additional_investment = carbon_taxes_zar
...
# Function to apply reinvestment using LHS
def apply_reinvestment_with_lhs(n, reinvestment_amount):
    invest_dict = {'onwind': 12708, 'solar': 8619}  # ZAR/kW for wind and solar
    renewable_carriers = ['onwind', 'solar']

    # Find generators that are extendable
    add_generators = n.generators[(n.generators['carrier'].isin(renewable_carriers)) & (n.generators.p_nom_extendable)]

    # If no generators are found, skip
    if add_generators.empty:
        print("No extendable renewable generators found.")
        return

    # Assign investment costs per MW
    add_generators['investment_cost'] = add_generators['carrier'].map(invest_dict) * 1000  # ZAR/MW

    # Create the LHS expression for the investment constraint
    generators_p_nom = get_var(n, "Generator", "p_nom")
    lhs = linexpr((add_generators['investment_cost'], generators_p_nom[add_generators.index])).sum()

    # Define the constraint to enforce total investment
    define_constraints(n, lhs, ">=", reinvestment_amount, 'Generator', 'additional_carbontax_investment')
...
#- here only adding re - not finding equilibrium -> fix? fixed :)
#def reinvestment_loop(n, base_investment, max_iterations=3, convergence_threshold=1e-3):
def reinvestment_loop(n, base_investment, config, opts, tmpdir, solver_logfile):
...
#unsure if i should use
        ...
        #To check
        print(f"Reinvestment loop applied: Base Investment = {base_investment} ZAR, "
        f"Additional Investment = {carbon_taxes_zar} ZAR, "
        f"Total Investment = {total_investment} ZAR")
...
#     # print("\033[91mStarting one time reinvestment...\033[0m")
...
#     print(f"Total CO2 Emissions: {total_emissions_mt} Mt")
    ...
    # Recalculate CO2 emissions after the reinvestment
    total_emissions = (n.generators_t.p * n.generators.carrier.map(n.carriers['co2_emissions'])).sum().sum()
    total_emissions_mt = total_emissions / 1e6  # Convert to Megatonnes
    print(f"Total CO2 Emissions after reinvestment: {total_emissions_mt} Mt")
